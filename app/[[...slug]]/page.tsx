...
const STATIC = ["", "product", "product/console", "product/ask-nekaf", "product/agents", "product/integrations", "pricing", "specialties", "customers", "about", "resources", "careers", "demo", "privacy", "terms", "safety-compliance", "cookies"];
...
function metaFor(path: string, lang: string, site: ReturnType<typeof getSite>) {
  ...
  const map: Record<string, { title: string; description: string }> = {
    "/product": site.product.meta, "/product/console": site.consolePage.meta, "/product/ask-nekaf": site.askNekaf.meta, "/product/agents": site.agentsPage.meta, "/product/integrations": site.integrations.meta,
...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: m.title, description: m.description, siteName: "Nekaf", locale, type: "website", images: [{ url: og, width: 1200, height: 630, alt: m.title }] },
...
function jsonLd(path: string, lang: string, site: ReturnType<typeof getSite>) {
  const org = { "@type": "Organization", "@id": `${BASE}/#org`, name: "Nekaf", url: BASE, logo: `${BASE}/og/${lang}.png`, contactPoint: [{ "@type": "ContactPoint", telephone: "+1-435-324-0610", contactType: "sales", areaServed: "US", availableLanguage: ["en"] }, { "@type": "ContactPoint", telephone: "+31-970-065-32593", contactType: "sales", areaServed: "NL", availableLanguage: ["nl", "en"] }], sameAs: ["https://www.linkedin.com/company/nekaf"], description: "Nekaf provides AI voice agents for automotive service businesses: independent garages, dealerships, body shops and tire centers in the Netherlands, Germany and the United States. The agent answers customer calls, books and moves service appointments, tells callers when their car is ready and routes breakdowns by the workshop's own rules.", founder: [{ "@id": `${BASE}/about#peter-paul-de-leeuw` }, { "@id": `${BASE}/about#thomas-brits` }], address: [{ "@type": "PostalAddress", addressLocality: "Amsterdam", addressCountry: "NL" }], areaServed: ["NL", "DE", "US", "EU"] };
  const pageUrl = `${BASE}${lang === "en" ? "" : "/" + lang}${path === "/" ? "/" : path}`;
  const meta = metaFor(path, lang, site);
  const website = { "@type": "WebSite", "@id": `${BASE}/#website`, url: BASE, name: "Nekaf", publisher: { "@id": `${BASE}/#org` }, inLanguage: ["en", "nl", "de"] };
  ...
  const founders = site.about.founders.people.map((f) => ({ "@type": "Person", "@id": `${BASE}/about#${f.name.toLowerCase().replace(/\s+/g, "-")}`, name: f.name, jobTitle: f.role, description: f.text, image: new URL(BASE).origin + f.image, worksFor: { "@id": `${BASE}/#org` }, url: `${BASE}${lang === "en" ? "" : "/" + lang}/about` }));
  const graph: Record<string, unknown>[] = [org, website, webpage, ...founders];
  const seg = path.split("/").filter(Boolean);
  const prefix = lang === "en" ? "" : "/" + lang;
  const label = (i: number): string => {
    const p = "/" + seg.slice(0, i + 1).join("/");
    const known: Record<string, string> = { "/product": site.product.hero.title, "/product/console": site.consolePage.hero.title, "/product/ask-nekaf": site.askNekaf.hero.title, "/product/agents": site.agentsPage.hero.title, "/product/integrations": site.integrations.hero.title, "/pricing": site.pricing.title, "/specialties": site.specialties.title, "/customers": site.customers.title, "/about": site.about.title, "/resources": site.resources.title, "/careers": site.careers.title, "/demo": site.demo.title };
  ...
  if (seg.length) graph.push({ "@type": "BreadcrumbList", "@id": `${pageUrl}#breadcrumb`, itemListElement: [{ "@type": "ListItem", position: 1, name: "Nekaf", item: `${BASE}${prefix}/` }, ...seg.map((_, i) => ({ "@type": "ListItem", position: i + 2, name: label(i), item: `${BASE}${prefix}/${seg.slice(0, i + 1).join("/")}` }))] });
  ...
  if (path === "/" || path.startsWith("/product") || path === "/pricing") graph.push({ "@type": "Service", "@id": `${BASE}/#service`, name: "Nekaf AI voice agents", provider: { "@id": `${BASE}/#org` }, serviceType: "AI voice agent for automotive service businesses", areaServed: ["US", "NL", "DE"], description: site.home.meta.description, offers: { "@type": "Offer", price: "299", priceCurrency: "USD", priceSpecification: { "@type": "UnitPriceSpecification", price: "299", priceCurrency: "USD", unitText: "MONTH" } } });
  if (path === "/" || path.startsWith("/product") || path === "/pricing") graph.push({ "@type": "SoftwareApplication", "@id": `${BASE}/#app`, name: "Nekaf", applicationCategory: "BusinessApplication", applicationSubCategory: "AI voice agent for garages and dealerships", operatingSystem: "Web", url: `${BASE}${lang === "en" ? "" : "/" + lang}/product`, description: meta.description, provider: { "@id": `${BASE}/#org` }, offers: { "@type": "Offer", price: lang === "en" ? "299" : "279", priceCurrency: lang === "en" ? "USD" : "EUR", url: `${BASE}${lang === "en" ? "" : "/" + lang}/pricing`, description: site.pricing.from } });
...
export default async function Page({ params }: { params: Promise<Params> }) {
  ...
  switch (path) {
    ...
    case "/product/ask-nekaf": return <AskPage {...ctx} />;
