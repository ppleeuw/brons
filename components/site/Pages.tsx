...
export function HomePage({ site, lang, path }: Ctx) {
  ...
  return (
    <Shell site={site} lang={lang} path={path} navVariant="transparent">
      <Hero h={h.hero} lang={lang} tel={site.meta.telDisplay} quote={h.quotes.items.find((q) => (lang === "en" ? /legacy/i : /nuland/i).test(q.logoAlt)) || h.quotes.items[0]} />
...
export function AskPage({ site, lang, path }: Ctx) {
  return <Shell site={site} lang={lang} path={path}>{productBody(site.askNekaf, lang, undefined, path)}</Shell>;
}
...
export function NotFound({ site, lang, path }: Ctx) {
  return (
    <Shell site={site} lang={lang} path={path}>
      <Section z={1}><div className="flex flex-col items-center gap-6 py-24 text-center"><h1 className="text-headline-lg text-black">404</h1><a className={BTN_PRIMARY + SM} href={lhref(lang, "/")}>Nekaf</a></div></Section>
    </Shell>
  );
}
export { ARTICLES };
