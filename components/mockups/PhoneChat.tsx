"use client";
import { AgentAvatar, AskBar, Bubble, CarIcon, CheckIcon, Pill, WrenchIcon } from "./ui";

export type Msg = { side: "agent" | "user"; text: string; widget?: "slots" | "order" | "plan" | "loaner" | "reminder" | "quote" };

/** Small in-chat widgets (workshop flavoured). */
export function Widget({ kind, dark = false }: { kind: NonNullable<Msg["widget"]>; dark?: boolean }) {
      ...
      <div className={"mt-1 max-w-[86%] rounded-[16px] rounded-tl-[4px] p-2.5 " + box}>
        <div className="flex items-center gap-2">
          ...
          <div className="text-[10.5px] leading-tight">
            <div className="font-medium">Front brake pads and discs · OEM set</div>
            <div className="opacity-60">Plate 472-KLM · 84,200 mi</div>
          </div>
        </div>
        <div className="mt-2 flex items-center justify-between text-[10px]">
          <span className="opacity-60">Arrives Fri 16 May</span>
          <span className="rounded-full bg-green-50 px-2 py-0.5 text-green-500">Ordered</span>
        </div>
      </div>
    );
  if (kind === "plan")
    return (
      <div className={"mt-1 max-w-[86%] rounded-[16px] rounded-tl-[4px] p-2.5 text-[10.5px] " + box}>
        <div className="mb-1 flex justify-between"><span className="opacity-60">Service plan</span><span>Care Plus</span></div>
        <div className="mb-1 flex justify-between"><span className="opacity-60">Annual balance</span><span>$180</span></div>
        <div className="mb-1 flex justify-between"><span className="opacity-60">Loaner car</span><span>Included</span></div>
        ...
        <div className="mt-1 opacity-60">$122 of $180 balance used</div>
      </div>
    );
  if (kind === "loaner")
    return (
      <div className={"mt-1 max-w-[86%] rounded-[16px] rounded-tl-[4px] p-2.5 " + box}>
        <div className="mb-1.5 text-[10px] opacity-60">Loaner cars available today</div>
        <div className="grid grid-cols-3 gap-1.5">
          {["Compact · Auto", "Wagon · Manual", "Van · 3 seats"].map((f, i) => (
            <div key={f} className={"rounded-lg p-1.5 text-center text-[9px] " + (dark ? "bg-white/10" : "bg-gray-100")}>
              <svg viewBox="0 0 40 18" className="mx-auto mb-1 h-4 w-9" fill="none" aria-hidden="true">
                <path d={i === 0 ? "M3 13V9.5h5l4.5-5h9l6 5h9.5V13" : i === 1 ? "M3 13V9.5h4l4-5h13l4 5h9V13" : "M3 13V9.5l3-5h22l9 5V13"} stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round" />
                <circle cx="11" cy="13" r="2.4" stroke="currentColor" strokeWidth="1.4" />
                <circle cx="29" cy="13" r="2.4" stroke="currentColor" strokeWidth="1.4" />
              </svg>
              {f}
            </div>
          ))}
  ...
  if (kind === "quote")
    return (
      <div className={"mt-1 max-w-[86%] rounded-[16px] rounded-tl-[4px] p-2.5 text-[10.5px] " + box}>
        <div className="mb-1 font-medium">Brake job quote</div>
        <div className="flex items-center justify-between"><span className="opacity-60">Front pads and discs · Marco</span><Pill tone="ghost">$489</Pill></div>
        <div className="mt-1.5 flex items-center gap-1 text-green-500"><CheckIcon className="h-3 w-3" /> Parts in stock, about 2 hours</div>
      </div>
    );
  return (
    <div className={"mt-1 max-w-[86%] rounded-[16px] rounded-tl-[4px] p-2.5 text-[10.5px] " + box}>
      <div className="flex items-center gap-2">
        <span className="flex h-7 w-7 items-center justify-center rounded-full bg-green-50 text-green-500"><CarIcon className="h-4 w-4" /></span>
        <div>
          <div className="font-medium">Inspection reminder set</div>
...
export default function PhoneChat({ brand = "Ridgeline Auto Care", messages, dark = false, className = "", bare = false, tint = "#f6f5f3" }: { brand?: string; messages: Msg[]; dark?: boolean; className?: string; bare?: boolean; tint?: string }) {
  const body = (
    <div className={"flex h-full w-full flex-col " + (dark ? "bg-[#1b2624] text-white" : "text-gray-700")} style={dark ? undefined : { background: tint }}>
      ...
      <div className={"mx-3 mt-2 flex items-center gap-2 rounded-[14px] px-3 py-2 text-[11px] " + (dark ? "bg-white/10" : "bg-white ring-1 ring-black/[0.05]")}>
        ...
        <div className="leading-tight">
          ...
          <div className="text-[9px] opacity-60">Service desk · online</div>
        </div>
      </div>
