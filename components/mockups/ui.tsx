/**
 * Shared primitives for the workshop product mockups. Shape language deliberately differs from the source
...
/** Open-end wrench, head to the upper right. */
export const WrenchIcon = ({ className = "h-3.5 w-3.5", style }: { className?: string; style?: CSSProperties }) => (
  <svg viewBox="0 0 24 24" fill="none" className={className} style={style} aria-hidden="true">
    <path d="M10.3 2.8v3.7h3.4V2.8a4.5 4.5 0 0 1 0 8.4v8.3a1.7 1.7 0 0 1-3.4 0v-8.3a4.5 4.5 0 0 1 0-8.4Z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" transform="rotate(45 12 12)" />
  </svg>
);

/** Car seen from the front: cabin, body and two wheels. */
export const CarIcon = ({ className = "h-3.5 w-3.5", style }: { className?: string; style?: CSSProperties }) => (
  <svg viewBox="0 0 24 24" fill="none" className={className} style={style} aria-hidden="true">
    <path d="M5.5 12.5l1.6-4.3A2 2 0 0 1 9 6.9h6a2 2 0 0 1 1.9 1.3l1.6 4.3" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M3.5 12.5h17a1 1 0 0 1 1 1v3.3h-19v-3.3a1 1 0 0 1 1-1Z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" />
    <circle cx="7.2" cy="17.4" r="1.7" stroke="currentColor" strokeWidth="1.6" />
    <circle cx="16.8" cy="17.4" r="1.7" stroke="currentColor" strokeWidth="1.6" />
  </svg>
);
...
/** Agent avatar: the Nekaf tire mark inside a soft squircle (used in the product mockups). */
      ...
      {title && (
        <div className={"flex items-center justify-between px-4 pt-3 pb-2 text-[11px] font-medium " + (dark ? "text-white/80" : "text-gray-400")}>
          <span className="flex items-center gap-1.5">
            <WrenchIcon className="h-3 w-3" style={{ color: accent } as CSSProperties} />
            {title}
          </span>
          {right}
        </div>
...
export const AskBar = ({ placeholder = "Ask Ridgeline Auto Care anything…", dark = false, className = "" }: { placeholder?: string; dark?: boolean; className?: string }) => (
