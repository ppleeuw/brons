import { AgentAvatar, AskBar, Bubble, Card, CarIcon, CheckIcon, Line, Pill, WrenchIcon, dottedBg } from "./ui";
  ...
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
      <Card title="Ghostwriter" accent="#4584c6" right={<Pill tone="ghost">Merge</Pill>} className="w-full">
        <div className="rounded-[14px] rounded-tl-[4px] bg-blue-100 px-3 py-2 text-[11px] text-blue-700">Create a service desk agent for Ridgeline Auto Care that books services, orders parts and checks service plans</div>
        <p className="mt-3 text-[11px] text-gray-400">
          Using <span className="text-blue-700">Journey generation</span>, <span className="text-blue-700">Test generation</span> and <span className="text-blue-700">Planner lookup</span>
        </p>
        <p className="mt-2 text-[11px] text-gray-400">The agent is built. Running the workshop validation step so it can catch any quote or planner issues.</p>
        <ul className="mt-3 flex flex-col gap-1.5">
          <Row>Updating agent · <span className="text-blue-700">View changes</span></Row>
          <Row>Researching parts brands</Row>
...
export function Recommendations({ className = "" }: { className?: string }) {
  const items = ["Add a same-week slot flow for callers who mention a warning light", "Offer a seasonal tire swap when a stored set is 60 days from its date", "Route quote questions to the price list lookup before quoting prices"];
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
        ...
        <ul className="flex flex-col gap-2">
          {items.map((t, i) => (
            <li key={i} className={"flex items-start gap-2 rounded-[14px] rounded-tl-[4px] px-3 py-2 text-[11px] " + (i === 1 ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-700")}>
              <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-[38%] bg-white text-green-500 ring-1 ring-black/[0.06]"><WrenchIcon className="h-2.5 w-2.5" /></span>
              {t}
            </li>
          ))}
        </ul>
...
export function TraceTimeline({ className = "" }: { className?: string }) {
  const steps = [["Transcription", "#4584c6", 12, 30], ["Initialize", "#7644a6", 34, 18], ["Supervisor: quote check", "#4faf62", 44, 40], ["Intent: customer asks to skip inspection", "#e94e2a", 60, 34], ["Response", "#4faf62", 78, 20]] as const;
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
      <Card title="Agent trace · service booking" accent="#7644a6" className="w-full">
  ...
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
      <Card title="Explorer report" accent="#4584c6" className="w-full">
        <div className="text-[12px] font-medium text-gray-700">“Why are customers transferred to a technician?”</div>
        <div className="mt-2 text-[10px] uppercase tracking-widest text-gray-350">Conversation highlights</div>
        <p className="mt-1 text-[11px] text-gray-400">Out of 118k conversations, the top reason for a hand-off was quotes older than 30 days.</p>
        <div className="mt-2 rounded-[12px] rounded-tl-[4px] bg-gray-100 px-3 py-2 text-[11px] text-gray-700">Expired quote</div>
...
export function ChatBubblesCard({ className = "" }: { className?: string }) {
  return (
    <div className={"flex h-full w-full flex-col justify-center gap-3 bg-blue-100 p-[10%] " + className}>
      <Bubble name="Agent">Hi Sarah, ready to pick up your car?</Bubble>
      <Bubble name="Agent" className="pl-6">Hello Sarah, how is the car running this evening?</Bubble>
    </div>
  );
}
...
export function ReasoningPanel({ className = "" }: { className?: string }) {
  const rows = [["Supervisors", ["Quote validity", "Parts availability"]], ["Decisions", ["Route to technician: no"]], ["Responses", ["Offer earliest service slot"]]] as const;
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
      ...
      <Card title="Memory" accent="#7644a6" className="w-full">
        <div className="flex items-center gap-2">
          ...
          <div className="leading-tight">
            <div className="text-[12px] font-medium text-gray-700">Sarah Miller</div>
            <div className="text-[10px] text-gray-350">sarah@example.com · customer since 2019</div>
          </div>
        </div>
        <ul className="mt-3 flex flex-col gap-1.5">
          <Row dot="bg-purple-600">Reason for calling: tire change</Row>
          <Row dot="bg-purple-600">Plate: 472-KLM, 84,200 mi, serviced Mar 2026</Row>
...
export function HistoryCard({ className = "" }: { className?: string }) {
  const rows = [["Today", "Booked inspection", "#4faf62"], ["Mar 12", "Ordered brake pads", "#4584c6"], ["Feb 3", "Tire change", "#7644a6"], ["Jan 20", "Service plan check", "#ebc247"]];
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
...
/** Integrations: workshop systems around a hub. */
export function IntegrationsCard({ className = "" }: { className?: string }) {
  const nodes = [["DMS", "#4584c6"], ["Parts", "#4faf62"], ["Insurer", "#7644a6"], ["POS", "#f96205"], ["CRM", "#ebc247"]];
  return (
    <div className={"relative flex h-full w-full items-center justify-center bg-blue-100 " + className} style={dottedBg}>
      <div className="relative aspect-square h-[72%] max-w-[72%]">
        ...
        <span className="absolute left-1/2 top-1/2 flex h-14 w-14 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-[38%] bg-blue-600 text-white shadow-[0_18px_30px_-16px_rgba(52,94,178,0.7)]"><WrenchIcon className="h-6 w-6" /></span>
      </div>
    </div>
  );
}
...
export function StrategyCard({ className = "" }: { className?: string }) {
  return (
    <div className={"flex h-full w-full flex-col justify-center gap-3 bg-blue-100 p-[8%] " + className}>
      <Card title="Audience" accent="#4584c6"><div className="text-[12px] text-gray-700">Lapsed service customers <span className="text-gray-350">(84k)</span></div></Card>
      <Card title="Outcome" accent="#4faf62"><div className="text-[12px] text-gray-700">Rebooked service & tire swap</div><div className="mt-2 flex gap-2"><Pill tone="green">Lifetime value</Pill><Pill tone="ghost">Retention</Pill></div></Card>
    </div>
  );
}

/** Workflow / Horizon "convert before" card. */
export function WorkflowCard({ className = "", title = "Book before the inspection lapses" }: { className?: string; title?: string }) {
  const steps = [["Goal", "Book the inspection before the current one expires", "#4faf62", false], ["Signals", "Inspection expires in 10 days · service overdue", "#4584c6", false], ["Reasoning", "Customer prefers evenings; offer Thursday 6pm slot", "#345eb2", true], ["Next actions", "Send reminder · Hold appointment · Confirm loaner", "#7644a6", false]] as const;
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
...
export function ActivityCard({ className = "" }: { className?: string }) {
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
      <Card title="Sarah Miller" accent="#4584c6" right={<Pill tone="ghost">Scheduled</Pill>} className="w-full">
        <div className="text-[10px] uppercase tracking-widest text-gray-350">Upcoming activity</div>
        <div className="mt-1 flex items-center justify-between rounded-[12px] bg-blue-100 px-3 py-2 text-[11px]"><span className="text-blue-700">Callback: brake job follow-up</span><span className="text-gray-350">Today 4:00 PM</span></div>
        <div className="mt-3 text-[10px] uppercase tracking-widest text-gray-350">Previous activity</div>
        {[["Outbound call", "Inspection reminder", "Jul 9th"], ["Inbound chat", "Order brake pads", "Jul 2nd"]].map(([a, b, d]) => (
...
export function JourneyList({ className = "", items = ["Book a service", "Seasonal tire change", "Check service plan", "Car-ready status call", "Inspection reminder"], active = 1 }: { className?: string; items?: string[]; active?: number }) {
  return (
        ...
        <ul className="flex flex-col gap-1">
          {items.map((it, i) => (
            <li key={it} className={"flex items-center justify-between rounded-[12px] px-3 py-2 text-[11px] " + (i === active ? "bg-blue-600 text-white" : "text-gray-700")}>
              <span className="flex items-center gap-2"><CarIcon className="h-3.5 w-3.5" />{it}</span>
  ...
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className} style={dottedBg}>
      <Card title="Knowledge base lookup" accent="#4faf62" className="w-full">
        ...
        <div className="rounded-[12px] rounded-tl-[4px] bg-gray-100 px-3 py-2 text-[11px] text-gray-700">“Can I keep driving with the check engine light on?”</div>
        <div className="mt-2 text-[10px] text-gray-350">Message</div>
        <div className="rounded-[12px] rounded-tl-[4px] bg-blue-100 px-3 py-2 text-[11px] text-blue-700">“How long can I drive with a warning light on?”</div>
...
export function SimulationsList({ className = "" }: { className?: string }) {
  const rows = ["Expired quote approval", "Lost or broken key", "Insurer not on the list", "Service reschedule same day", "Noise after repair report", "Wheel alignment upsell"];
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className} style={dottedBg}>
  ...
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className} style={dottedBg}>
      <Card title="Brand" accent="#4584c6" className="w-full">
        ...
        <div className="mb-2 rounded-[12px] bg-gray-100 px-3 py-2 text-[11px] text-gray-700">Ridgeline Auto Care Service Desk</div>
        <div className="text-[10px] text-gray-350">Tone of voice</div>
        <div className="mb-3 rounded-[12px] bg-gray-100 px-3 py-2 text-[11px] text-gray-700">Warm, reassuring and clear about car safety</div>
...
export function AccountCard({ className = "", title = "Customer details", lines = ["Send all of your customer’s vehicle details.", "Encrypted in transit and at rest."], light = false }: { className?: string; title?: string; lines?: string[]; light?: boolean }) {
  return (
  ...
  return (
    <div className={"flex h-full w-full items-center justify-center bg-gray-100 " + className}>
      <div className="relative w-[78%] rounded-[22px] bg-white p-5 ring-1 ring-black/[0.06]">
        ...
        <div className="flex items-center justify-between text-[12px] text-gray-700"><span>Our workshop policies</span><span className="flex h-6 w-6 items-center justify-center rounded-full bg-gray-100 text-gray-400"><WrenchIcon className="h-3 w-3" /></span></div>
  ...
  return (
    <div className={"relative h-full w-full bg-blue-100 " + className} style={dottedBg}>
      ...
      <div className="absolute left-[42%] top-[64%] rounded-full bg-white px-3 py-1.5 text-[10px] text-gray-700 shadow-[0_10px_20px_-12px_rgba(15,79,138,0.5)] ring-1 ring-black/[0.06]">
        <span className="mr-1 text-blue-700">LLM:</span> choose voice model
      </div>
    </div>
  );
}
  ...
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
      <Card title="Reschedule request" accent="#4584c6" className="w-full">
        <div className="text-[10px] text-gray-350">Garage De Vries · to: sarah@example.com</div>
        <p className="mt-2 text-[11px] leading-snug text-gray-700">Hi Sarah,</p>
        <p className="mt-1 text-[11px] leading-snug text-gray-400">Happy to help reschedule your service. Here are a few options that work with your service plan:</p>
...
export function ChatAppCard({ className = "" }: { className?: string }) {
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
      <Card title="Ridgeline Auto Care · in ChatGPT" accent="#4faf62" className="w-full">
        <div className="rounded-[12px] rounded-tr-[4px] bg-gray-100 px-3 py-2 text-[11px] text-gray-700">Looking for all-season tires for a 2021 Golf</div>
        <div className="mt-2 grid grid-cols-3 gap-1.5">
          {[["All-season · Touring", "$189"], ["All-season · Eco", "$149"], ["All-season · Value", "$129"]].map(([n, p], i) => (
            <div key={n} className="rounded-[12px] bg-white p-2 ring-1 ring-black/[0.06]">
              <div className="mb-1.5 flex h-8 items-center justify-center rounded-md bg-green-50 text-green-500"><CarIcon className="h-5 w-5" /></div>
              <div className="text-[9px] text-gray-700">{n}</div>
              <div className="text-[9px] text-gray-350">{p}{i === 0 ? " · in stock" : ""}</div>
            </div>
          ))}
        </div>
        <div className="mt-3"><AskBar placeholder="Ask about tires or parts…" /></div>
      </Card>
    </div>
  );
}

/** Customer service UI with customer detail sidebar. */
export function ServiceUI({ className = "" }: { className?: string }) {
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
      <Card title="Customer: Sarah Miller" accent="#4584c6" right={<Pill tone="ghost">Verified</Pill>} className="w-full">
        <div className="text-[10px] text-gray-350">Reason for contact</div>
        <div className="text-[11px] text-gray-700">Brake job appointment · Positive sentiment</div>
        <ul className="mt-2 flex flex-col gap-1.5">
          <Row>Verify customer identity and plan type</Row>
          <Row dot="bg-blue-600">Offer to book a slot at the nearest branch</Row>
          <Row dot="bg-gray-250">Confirm out-of-pocket amount</Row>
        </ul>
        <div className="mt-3 rounded-[12px] rounded-tl-[4px] bg-blue-100 px-3 py-2 text-[11px] text-blue-700">Your plan covers an inspection every 12 months: the next one is free.</div>
      </Card>
    </div>
  );
}
...
export function BriefingCard({ className = "", compact = false }: { className?: string; compact?: boolean }) {
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
      {compact ? (
        <div className="rounded-full bg-purple-100 px-4 py-2 text-[12px] text-purple-500 ring-1 ring-purple-400/40"><span className="mr-1">✦</span> Here is your weekly workshop briefing.</div>
      ) : (
        <Card title="Weekly briefing" accent="#7644a6" className="w-full">
          ...
          <p className="mt-1 text-[11px] text-gray-700">Service bookings up 9% after reminder texts; parts orders dipped 4% where stock ran low.</p>
...
/** "Get insights from customers" query card. */
export function InsightsQuery({ className = "" }: { className?: string }) {
  const qs = ["How should we improve the agent’s tire change flow?", "What content should we add to the workshop knowledge base?", "Which customer intents are rising this month?", "Find conversations where a customer mentioned a warning light"];
  return (
    <div className={"flex h-full w-full items-center justify-center bg-blue-100 p-[8%] " + className}>
      <Card title="Get insights from customers" accent="#4584c6" className="w-full">
        <div className="flex items-center justify-between rounded-[12px] bg-gray-100 px-3 py-2 text-[11px] text-gray-700"><span>Why did parts orders drop last week?</span><Pill tone="green">Explore</Pill></div>
        <ul className="mt-2 flex flex-col gap-1">
          {qs.map((q) => (
...
export function PricingCard({ className = "" }: { className?: string }) {
  const rows: [string, string][] = [["Calls answered", "1,204"], ["Appointments booked", "212"], ["No-shows recovered", "38"], ["Car-ready status confirmed", "156"]];
  return (
